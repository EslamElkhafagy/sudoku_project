...
public class Sudoku
{
	 ...
	 */
	public static ArrayList<Integer> getAllMoves(int sudoku[][],int row, int column) {
...
public	static boolean isFull(int sudoku[][]) {
	 ...
	 *         cell
	 */
	public static int[] getFirstEmptyCell(int sudoku[][]) {
		for (int i = 0; i < 9; ++i) {
			for (int j = 0; j < 9; ++j) {
				if (sudoku[i][j] == 0) {
					int ans[] = { i, j };
...
public static int [][] solve(int sudoku[][]) {
		...
		// get the first empty cell
		int cell[] = getFirstEmptyCell(sudoku);
		int row = cell[0];
		int column = cell[1];

		// get all valid moves of this cell
		ArrayList<Integer> validMoves = getAllMoves(sudoku,row, column);
